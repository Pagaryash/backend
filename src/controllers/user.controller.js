...
const registerUser = asyncHandler(async (req, res) => {
  ...
  // remove password and refresh token filed from user
  ...
  const createdUser = await User.findById(user._id).select(
    "-password -refreshToken" // rejected filed with -sign and in quotes
  );
  if (!createdUser) {
...
const loginUser = asyncHandler(async (req, res) => {
  ...
  const isPasswordValid = await user.isPasswordCorrect(password); // not capital User it is a mongodb mongoose method, but smaller is user has own defined method
  if (!isPasswordValid) {
...
const updateAccountDetails = asyncHandler(async (req, res) => {
  ...
  const user = await User.findByIdAndUpdate(
    ...
    {
      $set: {
        // set is a parameters that receive a object
        fullName: fullName,
        email: email,
      },
    },
    { new: true } // it will return updated info
  ).select("-password");
  return res
    .status(200)
